...
async def main():
    ...
    num_proxies_to_use = min(100, len(all_proxies))
    active_proxies = random.sample(all_proxies, num_proxies_to_use)  # Number of proxies to use
    ...
    while True:
        ...
        for task in done:
            if task.result() is None:
                ...
                if len(all_proxies) > len(active_proxies):
                    new_proxy = random.choice(list(set(all_proxies) - set(active_proxies)))
